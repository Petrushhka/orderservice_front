...
const MyPage = () => {
  ...
  const fetchMemberInfo = async () => {
    try {
      ...
      // ADMIN인 경우는 애초에 리스트로 리턴, 일반회원은 직접 배열로 감싸주자.(고차함수 돌려야 되니까)
      ...
      setMemberInfoList((prev) => {
        return data.map((user) => [
          ...
          {
            key: '도시',
            value: user.address?.city || '등록 전입니다.',
          },
          {
            key: '상세주소',
            value: user.address?.street || '등록 전 입니다.',
          },
          {
            key: '우편번호',
            value: user.address?.zipCode || '등록 전 입니다.',
          },
        ]);
      });
    } catch (e) {
