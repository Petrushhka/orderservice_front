...
// 요청용 인터셉터
// 인터셉터의 use합수는 매개값 두 개 받습니다. 둘 다 콜백 함수 형식 입니다.
...
axiosInstance.interceptors.request.use(
  ...
  (error) => {
    console.log(error);
    Promise.reject(error); // reject가 호출되면 비동기가 취소 된다.
  },
);

// 응답용 인터셉터 설정
axiosInstance.interceptors.response.use(
  (response) => response, // 응답에  문제가 없다면 그대로 응답 객체를 리턴.
  async (error) => {
    console.log('response interceptor가 동작함!, 응답에 문제가 발생');
    console.log(error);

    if (error.response.data.message === 'NO_LOGIN') {
      console.log('로그인을 하지 않아서 재발급 요청 들어갈 수 없음.');
      return Promise.reject(error);
    }

    // 원래의 요청 정보를 기억해 놓자 -> 새 토큰 발급 받아서 다시 시도할거니까.
    ...
    if (error.response.status === 401) {
      ...
      try {
        ...
        const newToken = res.data.result.token; // axios는 json() -> x 안씁니다.
        ...
        // axiosInstance를 사용하여 다시 한 번 원본요청을 보내고, 응답은 원래 호출한 곳으로 리턴
        return axiosInstance(originalRequest);
      } catch (error) {
        console.log(error);
        // 백엔드에서 401을 보낸 상황 -> Refresh token 도 만료된 상황 (로그아웃처럼 처리해줘야 함.)
        localStorage.clear();
      }
    }
